...
var extractNyparking = function (callback) {
    ...
    var dest = path.join(process.cwd(), 'tmp', 'nyparking_signs.csv');
...
var unzipShapeFile = function (callback) {
    var stream = fs.createReadStream(config.shapeFile).pipe(unzip.Extract({ path:path.join(process.cwd(), 'tmp/')}));
